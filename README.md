...
В версиях до 0.1.3 архив собирался на Windows с обратными слэшами (`Zvuk\install.xml`). LMS на Linux (Daphile) не мог его распаковать, и плагин исчезал после перезапуска. Обновитесь до **v0.1.3** или новее.

### Если в настройках пустые подписи полей

В версиях до 0.1.4 файл `strings.txt` мог иметь окончания строк Windows (CRLF). LMS на Linux тогда не загружает переводы. Обновитесь до **v0.1.4** и перезапустите LMS.
